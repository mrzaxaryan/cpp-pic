/**
 * @file syscall.h
 * @brief OpenBSD syscall numbers and BSD type definitions.
 *
 * @details Defines OpenBSD syscall numbers, POSIX/BSD constants, file descriptor
 * flags, memory protection flags, socket options, errno values, and kernel
 * structures (OpenBsdDirent, Timespec, Pollfd). Syscall numbers are shared
 * across i386, x86_64, AArch64, and RISC-V 64 OpenBSD architectures. Many
 * constants match FreeBSD (shared BSD heritage) but some differ (AT_REMOVEDIR,
 * CLOCK_MONOTONIC, AF_INET6, and most syscall numbers).
 *
 * @see OpenBSD syscall.h
...
constexpr USIZE SYS_LSEEK      = 166;

// File operations
constexpr USIZE SYS_STAT       = 38;
constexpr USIZE SYS_FSTAT      = 53;
constexpr USIZE SYS_UNLINK     = 10;

// Directory operations
constexpr USIZE SYS_MKDIR      = 136;
constexpr USIZE SYS_RMDIR      = 137;
constexpr USIZE SYS_GETDENTS   = 99;

// *at syscalls
constexpr USIZE SYS_OPENAT     = 321;
constexpr USIZE SYS_UNLINKAT   = 325;
constexpr USIZE SYS_MKDIRAT    = 318;
constexpr USIZE SYS_FSTATAT    = 42;
constexpr SSIZE AT_FDCWD       = -100;
constexpr INT32 AT_REMOVEDIR   = 0x08;

// Memory operations
constexpr USIZE SYS_MMAP       = 49;
...
constexpr USIZE SYS_POLL       = 252;

// Time operations
constexpr USIZE SYS_CLOCK_GETTIME  = 87;
constexpr USIZE SYS_GETTIMEOFDAY   = 67;

// Process operations
constexpr USIZE SYS_EXECVE     = 59;
constexpr USIZE SYS_DUP2       = 90;
constexpr USIZE SYS_SETSID     = 147;
constexpr USIZE SYS_WAIT4      = 11;
constexpr USIZE SYS_PIPE2      = 101;
...
// Socket options (BSD values -- same as macOS/FreeBSD)
...
constexpr INT32 CLOCK_MONOTONIC = 3;
...
/// @brief OpenBSD directory entry returned by the getdents syscall.
struct OpenBsdDirent
{
	...
	UINT8 Namlen;  ///< Length of the filename in bytes (excluding null terminator)
	UINT8 Pad[4];  ///< Padding
