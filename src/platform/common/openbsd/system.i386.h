/**
 * @file system.i386.h
 * @brief i386 OpenBSD syscall implementation via inline assembly.
 *
 * @details Provides System::Call overloads (0-6 arguments) that invoke OpenBSD
 * syscalls using the int $0x80 trap gate. Like FreeBSD, OpenBSD passes
 * arguments on the stack with a dummy return address at ESP. The syscall
 * number is in EAX, and the carry flag indicates error (positive errno in
 * EAX). The inline assembly negates eax on error to normalize to the
