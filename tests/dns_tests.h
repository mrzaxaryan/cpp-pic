...
private:
	...
	static BOOL TestLocalhostResolution()
	{
		...
		if (!result)
		{
			LOG_ERROR("Localhost A resolution failed (error: %e)", result.Errors());
			return false;
		}
		...
		if (!result6)
		{
			LOG_ERROR("Localhost AAAA resolution failed (error: %e)", result6.Errors());
			return false;
		}
	...
	static BOOL TestCloudflareResolve()
	{
		...
		if (!result)
		{
			LOG_ERROR("Cloudflare DNS resolution failed (error: %e)", result.Errors());
			return false;
		}
		IPAddress ip = result.Value();
	...
	static BOOL TestGoogleResolve()
	{
		...
		if (!result)
		{
			LOG_ERROR("Google DNS resolution failed (error: %e)", result.Errors());
			return false;
		}
		IPAddress ip = result.Value();
	...
	static BOOL TestMainResolve()
	{
		...
		if (!result)
		{
			LOG_ERROR("Main DNS resolution failed (error: %e)", result.Errors());
			return false;
		}
	...
	static BOOL TestKnownIpResolution()
	{
		...
		if (!result)
		{
			LOG_ERROR("DNS resolution for dns.google failed (error: %e)", result.Errors());
			return false;
		}
