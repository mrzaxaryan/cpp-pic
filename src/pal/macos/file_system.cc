...
File FileSystem::Open(PCWCHAR path, INT32 flags)
{
    ...
    // Map FileSystem flags to macOS open flags
    INT32 openFlags = 0;
    ...
    // Open the file using macOS open syscall
    SSIZE fd = Syscall::syscall3(SYS_OPEN, (USIZE)utf8Path, openFlags, mode);
    ...
    : handle((PVOID)INVALID_FD)
{
    ...
    (VOID)path;
    (VOID)currentEntry;  // Suppress unused warning
    first = TRUE;
}
