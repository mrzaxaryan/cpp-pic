 ...
 * @brief RISC-V 64-bit OpenBSD syscall implementation via inline assembly.
 *
 * @details Provides System::Call overloads (0-6 arguments) that invoke OpenBSD
 * syscalls using the ecall instruction. Like FreeBSD (and unlike Linux which
 * uses a7), OpenBSD RISC-V places the syscall number in t0 (x5). Arguments
 * are passed in a0-a5 (x10-x15). After ecall, t0 serves as the error
 * indicator: t0=0 on success, t0!=0 on error (a0 contains the positive
 ...
 *
 * @see OpenBSD riscv64 syscall.c
 *      https://github.com/openbsd/src/blob/master/sys/arch/riscv64/riscv64/syscall.c
 */
#pragma once
