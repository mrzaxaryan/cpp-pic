...
private:
	...
	static BOOL TestSecureWebSocketConnection()
	{
		...
		if (!openResult)
		{
			LOG_ERROR("Secure WebSocket handshake failed (error: %e)", openResult.Errors());
			return false;
		}
	...
	static BOOL TestWebSocketTextEcho()
	{
		...
		if (!openResult)
		{
			LOG_ERROR("WebSocket connection failed (error: %e)", openResult.Errors());
			return false;
		}
		...
		if (!writeResult)
		{
			LOG_ERROR("Failed to send message (error: %e)", writeResult.Errors());
			(void)wsClient.Close();
			return false;
		}
		...
		if (!readResult)
		{
			LOG_ERROR("Failed to receive echo response (error: %e)", readResult.Errors());
			(void)wsClient.Close();
			return false;
		}
	...
	static BOOL TestWebSocketBinaryEcho()
	{
		...
		if (!writeResult)
		{
			LOG_ERROR("Failed to send binary message (error: %e)", writeResult.Errors());
			(void)wsClient.Close();
			return false;
		}
	...
	static BOOL TestLargeMessage()
	{
		...
		if (!writeResult)
		{
			LOG_ERROR("Failed to send large message (error: %e)", writeResult.Errors());
			delete[] largeMessage;
			(void)wsClient.Close();
			return false;
		}
		...
		if (!readResult)
		{
			LOG_ERROR("Failed to receive large echo response (error: %e)", readResult.Errors());
			delete[] largeMessage;
			(void)wsClient.Close();
			return false;
		}
