 ...
 * @brief AArch64 OpenBSD syscall implementation via inline assembly.
 *
 ...
 * convention, same as Linux/FreeBSD, not svc #0x80 / x16 as on macOS).
 * Arguments are passed in x0-x5. Errors are indicated by the carry flag
 * (C bit in NZCV); the inline assembly negates x0 on error to normalize
 * to the negative-return convention. X1 is clobbered by the kernel's rval[1]
...
public:
	...
	// Note: OpenBSD kernel may write rval[1] to X1 on return, so X1 must be clobbered.
