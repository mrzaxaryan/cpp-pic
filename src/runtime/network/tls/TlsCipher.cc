...
PINT8 TlsCipher::CreateClientRand()
{
    ...
    for (UINT64 i = 0; i < sizeof(this->data12.clientRandom); i++)
    {
...
BOOL TlsCipher::ComputeKey(ECC_GROUP ecc, const CHAR *serverKey, INT32 serverKeyLen, PCHAR finishedHash)
{
    ...
    auto server_key = ecc == ECC_NONE ? "s ap traffic"_SS : "s hs traffic"_SS;
    auto client_key = ecc == ECC_NONE ? "c ap traffic"_SS : "c hs traffic"_SS;
    TlsHash hash2;
    ...
    if (ecc == ECC_NONE)
    {
        ...
        TlsHKDF::ExpandLabel(salt, hashLen, (UINT8 *)this->data13.pseudoRandomKey, hashLen, "derived"_SS, 7, hash, hashLen);
    ...
    else
    {
        ...
        TlsHKDF::ExpandLabel(salt, hashLen, this->data13.pseudoRandomKey, hashLen, "derived"_SS, 7, hash, hashLen);
    ...
    TlsHKDF::ExpandLabel(localKeyBuffer, keyLen, this->data13.handshakeSecret, hashLen, "key"_SS, 3, NULL, 0);
    TlsHKDF::ExpandLabel(localIvBuffer, this->chacha20Context.GetIvLength(), this->data13.handshakeSecret, hashLen, "iv"_SS, 2, NULL, 0);
    ...
    TlsHKDF::ExpandLabel(remoteKeyBuffer, keyLen, this->data13.mainSecret, hashLen, "key"_SS, 3, NULL, 0);
    TlsHKDF::ExpandLabel(remoteIvBuffer, this->chacha20Context.GetIvLength(), this->data13.mainSecret, hashLen, "iv"_SS, 2, NULL, 0);
