...
private:
	...
	static BOOL TestSecureWebSocketConnection()
	{
		...
		if (!openResult)
		{
			LOG_ERROR("Secure WebSocket handshake failed (error: %u)", openResult.ErrorAt(0).Code);
			return false;
		}
	...
	static BOOL TestWebSocketTextEcho()
	{
		...
		if (!openResult)
		{
			LOG_ERROR("WebSocket connection failed (error: %u)", openResult.ErrorAt(0).Code);
			return false;
		}
		...
		if (!writeResult)
		{
			LOG_ERROR("Failed to send message (error: %u)", writeResult.ErrorAt(0).Code);
			(void)wsClient.Close();
			return false;
		}
		...
		if (!readResult)
		{
			LOG_ERROR("Failed to receive echo response (error: %u)", readResult.ErrorAt(0).Code);
			(void)wsClient.Close();
			return false;
		}
	...
	static BOOL TestWebSocketBinaryEcho()
	{
		...
		if (!writeResult)
		{
			LOG_ERROR("Failed to send binary message (error: %u)", writeResult.ErrorAt(0).Code);
			(void)wsClient.Close();
			return false;
		}
	...
	static BOOL TestLargeMessage()
	{
		...
		if (!writeResult)
		{
			LOG_ERROR("Failed to send large message (error: %u)", writeResult.ErrorAt(0).Code);
			delete[] largeMessage;
			(void)wsClient.Close();
			return false;
		}
		...
		if (!readResult)
		{
			LOG_ERROR("Failed to receive large echo response (error: %u)", readResult.ErrorAt(0).Code);
			delete[] largeMessage;
			(void)wsClient.Close();
			return false;
		}
