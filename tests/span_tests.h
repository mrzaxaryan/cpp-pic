...
public:
	static BOOL RunAll()
	{
		...
		// Dynamic extent
		RunTest(allPassed, &TestDynamicDefault, "Dynamic default construction");
		RunTest(allPassed, &TestDynamicPtrSize, "Dynamic pointer+size construction");
		RunTest(allPassed, &TestDynamicArray, "Dynamic array construction");
		RunTest(allPassed, &TestDynamicAccessors, "Dynamic accessors");
		RunTest(allPassed, &TestDynamicSubspan, "Dynamic Subspan");
		RunTest(allPassed, &TestDynamicFirstLast, "Dynamic First/Last");
		RunTest(allPassed, &TestDynamicIteration, "Dynamic range-for iteration");
		RunTest(allPassed, &TestDynamicConstConversion, "Dynamic Span<T> to Span<const T>");

		// Static extent
		RunTest(allPassed, &TestStaticArray, "Static array construction");
		RunTest(allPassed, &TestStaticAccessors, "Static accessors");
		RunTest(allPassed, &TestStaticSubspan, "Static Subspan returns dynamic");
		RunTest(allPassed, &TestStaticFirstLast, "Static First/Last return dynamic");
		RunTest(allPassed, &TestStaticIteration, "Static range-for iteration");
		RunTest(allPassed, &TestStaticConstConversion, "Static Span<T,N> to Span<const T,N>");

		// Cross-conversion
		RunTest(allPassed, &TestStaticToDynamic, "Span<T,N> to Span<T> conversion");
		RunTest(allPassed, &TestStaticToDynamicConst, "Span<T,N> to Span<const T> conversion");
		RunTest(allPassed, &TestStaticPassToFunction, "Pass Span<T,N> to function taking Span<T>");

		// Size optimization
		RunTest(allPassed, &TestSizeOptimization, "Static extent eliminates size member");

		// Compile-time slicing — static extent
		RunTest(allPassed, &TestStaticPtrConstruction, "Static explicit pointer construction");
		RunTest(allPassed, &TestStaticCompileTimeFirst, "Static First<N>() returns Span<T,N>");
		RunTest(allPassed, &TestStaticCompileTimeLast, "Static Last<N>() returns Span<T,N>");
		RunTest(allPassed, &TestStaticCompileTimeSubspanOffsetCount, "Static Subspan<O,N>() returns Span<T,N>");
		RunTest(allPassed, &TestStaticCompileTimeSubspanOffset, "Static Subspan<O>() deduces count from type");

		// Compile-time slicing — dynamic extent
		RunTest(allPassed, &TestDynamicCompileTimeFirst, "Dynamic First<N>() returns Span<T,N>");
		RunTest(allPassed, &TestDynamicCompileTimeLast, "Dynamic Last<N>() returns Span<T,N>");
		RunTest(allPassed, &TestDynamicCompileTimeSubspan, "Dynamic Subspan<O,N>() returns Span<T,N>");

		// Edge cases
		RunTest(allPassed, &TestEmptyDynamic, "Empty dynamic span");
		RunTest(allPassed, &TestSingleElement, "Single element static span");
